...
draft: false
summary: Reduce the threat of crticial vulnerabilities such as Remote Code Execution (RCE) while cutting the size of your deployment by 90%
---

# Removing node_modules from your Server for Security and Size with Webpack
...
As the ecosystem has evolved, `node_modules` also became the focus of DevSecOps teams.
Packages include scripts that run on installation, provide telemetry, and include binaries.
While these get bundled away for client code, it is standard practice to upload the node_modules whole cloth to to server.
This isn't great.
...
Security is paramount to our clients, and the ability to deliver updates in the form of single, small Docker image allows us to ship frequent updates.
By using webpack to bundle our server, we provide our clients with a single, controlled entry point, support an array of clients all using separate CDNs, and shrink our Docker image size by 90%, which saves us time and money.

## Defining your entry points

For those new to the NodeJS ecosystem, it may be surprising to learn that installing a pacakge can execute a variety of scripts. The output of these scripts often includes an executable in `/node_modules/.bin`, which is delivered with the server code. Anyone with access to the machine can execute these scripts and we as developers have to accept that there are probably a few exploitable packages in there. What's worse, our application doesn't use 90% of these scripts in runtime.
...
Invariably, when attempting to bundle all your `node_modules` into your webpack build, you'll across a missing dependency.
...
While we could mitigate the error by installing the `pg-native` as an app dependency, it's not ideal since we aren't actually using the package. What's more, that pacakge may employ the same code pattern, causing you to import even more unnecessary packages.
...
For example, pacakges such as uWebSockets.js use an ARM64 binary when running on your Macbook, but require an AMD64 binary when running on linux in production.
...
Above we see webpack was able reference the `.node` binary correctly, ignoring the dozens of other binaries and extracting it into the output directory. True, strategy means that your build architecture must match your runtime architecture, but that is gneerally considered a best practice, anyways.
...
Take, for example, the famous `sharp` package used for image manipulation. It has a dependency on a vendor pacakge called `libvips`. When `sharp` is installed, it uses a file called `bindings.gyp` to build a reference to the included `libvips` binary. Unfortunately, that reference is built mimic the node_modules folder structure, not the structure of our output directory. Since building references between two binaries is outside the scope of JavaScript, we could either write a script to rewrite the bindings & reinstall, or we can adjust our output directory structure.
...
Notice that the name is prefixed with 2 directories. While this rule is applied to all binaries, we could easily target only `sharp` in the case of conflicting requirements.

### Substitution When Possible

While the above patterns work, the best option is often replacing node binaries with plain old JavaScript. For example, the secure hashing package `bcrypt` can be replaced with `bcryptjs`. True, you may sacrifice some performance, but that generally does not matter for anything except the most computationally expensive tasks.
...
Personally, I have found that this speeds up security audits as well. Common Vulnerabilities and Exposures (CVEs) that are identified based on dependencies can be safely ignored if the resulting code is not in the actual bundle. This is incredibly useful because all too often the CVEs identified are for pacakges that are only used in building, development, or testing.
